"""Train the conditional Flow Matching head on preprocessed nuPlan samples."""
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Train nuPlan flow-matching head.")
    p.add_argument("--data-path", type=str, default="data/nuplan")
    p.add_argument("--output", type=str, default="outputs/nuplan_flow/model.pt")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        print("[dry-run] flow train CLI parsed OK.")
        return 0
    ...
    state = train(
        ...
        scheduler=scheduler,
        cfg_metadata={"head": "flow", "args": vars(args)},
    )
    print(f"[train_nuplan_flow] last_loss={state.last_loss:.4f}", file=sys.stderr)
    return 0
