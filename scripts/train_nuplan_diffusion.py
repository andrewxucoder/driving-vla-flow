"""Train the conditional Diffusion head on preprocessed nuPlan samples."""
...
from driving_vla.models import ConditionalDiffusionModel, DrivingObservationEncoder
from driving_vla.policies import DiffusionPolicy
from driving_vla.training import TrainConfig, build_lr_scheduler, build_optimizer, train


def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Train nuPlan diffusion head.")
    p.add_argument("--data-path", type=str, default="data/nuplan")
    p.add_argument("--output", type=str, default="outputs/nuplan_diffusion/model.pt")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        print("[dry-run] diffusion train CLI parsed OK.")
        return 0
    ...
    policy = DiffusionPolicy(encoder=encoder, head=head, sampling_steps=args.sampling_steps).to(device)
    ...
    state = train(
        ...
        cfg_metadata={
            "head": "diffusion",
            # See diffusion_head module docstring: post-Round-0 the head switched
            # to x0 prediction. Loader uses this tag to refuse Round-0 ε ckpts
            # against the new sampler.
            "parameterization": "x0",
            "args": vars(args),
        },
    )
    print(f"[train_nuplan_diffusion] last_loss={state.last_loss:.4f}", file=sys.stderr)
    return 0
